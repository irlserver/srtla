...
void set_socket_buffers(int socket_fd) {
    ...
    if (setsockopt(socket_fd, SOL_SOCKET, SO_SNDBUF, &bufsize, sizeof(bufsize)) != 0) {
...
int main(int argc, char **argv) {
    ...
    args.add_argument("--srtla_port").help("Port to bind the SRTLA socket to").default_value(static_cast<uint16_t>(5000)).scan<'d', uint16_t>();
    args.add_argument("--srt_hostname").help("Hostname of the downstream SRT server").default_value(std::string{"127.0.0.1"});
    args.add_argument("--srt_port").help("Port of the downstream SRT server").default_value(static_cast<uint16_t>(4001)).scan<'d', uint16_t>();
    ...
    int resolve_result = srtla::utils::NetworkUtils::resolve_srt_address(
        srt_hostname.c_str(), srt_port.c_str(), &srt_addr, RECV_BUF_SIZE, SEND_BUF_SIZE);
    if (resolve_result < 0) {
        return EXIT_FAILURE;
    }
    ...
    int v6only = 0;
    if (setsockopt(srtla_sock, IPPROTO_IPV6, IPV6_V6ONLY, &v6only, sizeof(v6only)) < 0) {
    ...
    if (bind(srtla_sock, reinterpret_cast<const struct sockaddr *>(&listen_addr), sizeof(listen_addr)) < 0) {
        spdlog::critical("SRTLA socket bind failed");
        return EXIT_FAILURE;
    }

    if (srtla::utils::NetworkUtils::epoll_add(epoll_fd, srtla_sock, EPOLLIN, nullptr) != 0) {
    ...
    srtla::protocol::SRTHandler srt_handler(srtla_sock, srt_addr, epoll_fd, registry);
    srtla::protocol::SRTLAHandler srtla_handler(srtla_sock, registry, srt_handler, metrics_collector);
    ...
    const auto keepalive_callback = [&srtla_handler](const srtla::connection::ConnectionPtr &conn, time_t ts) {
        srtla_handler.send_keepalive(conn, ts);
    };
...
time_t ts = 0;
        ...
        for (int i = 0; i < eventcnt; i++) {
            ...
            if (events[i].data.ptr == nullptr) {
                ...
                auto raw_group = static_cast<srtla::connection::ConnectionGroup *>(events[i].data.ptr);
